...
Operator `OR` memastikan bahwa jika salah satu kondisi terpenuhi, baris tersebut akan muncul di hasil. Penting untuk menggunakan `IS NULL` (bukan `= NULL`) karena `NULL` adalah nilai khusus yang tidak bisa dibandingkan dengan operator `=`.
